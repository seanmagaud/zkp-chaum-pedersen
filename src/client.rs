...
async fn main() {
    ...
    buf.clear(); // will empty the memory string
    ...
    let request = RegisterRequest {user: username.clone(), y1: y1.to_bytes_be(), y2: y2.to_bytes_be()};
    ...
    let request = AuthenticationChallengeRequest {user: username.clone(), r1: r1.to_bytes_be(), r2: r2.to_bytes_be()};
    let response = client.create_authentication_challenge(request).await.expect("Could not request challenge in server").into_inner();
    ...
    let request = AuthenticationAnswerRequest {auth_id, s: s.to_bytes_be()};
